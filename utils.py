...
def plot_errorbar(
    xs, ys, error_lower, error_upper, colors='C0', error_width=12, alpha=.3
):
    ...
    for ii, (x, y, err_l, err_u) in enumerate(
        zip(xs, ys, error_lower, error_upper)
    ):
...
def simulate_2_parameter_bayesian_learning(
    x_obs, y_obs, 
    param_a_grid, param_b_grid, 
    true_param_a, true_param_b,
    model_func,
    posterior_func,
    ...
    for ii, idx in enumerate(sample_idx):
        ...
        plt.scatter(param_a, param_b, s=60, c=f'C{ii}', alpha=.75, zorder=20)
    ...
    plt.sca(axs[1])
    plt.scatter(x_obs, y_obs, s=60, c='k', alpha=.5)
    ...
    for ii, (param_a, param_b) in enumerate(zip(param_a_list, param_b_list)):
        ...
        plt.plot(xs, ys, color=f"C{ii}", linewidth=4, alpha=.5)
        
    groundtruth_ys = model_func(xs, true_param_a, true_param_b)
    plt.plot(xs, groundtruth_ys, color='k', linestyle='--', alpha=.5, label='true trend')
